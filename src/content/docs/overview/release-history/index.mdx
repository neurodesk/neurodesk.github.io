...
sidebar:
  order: 7
# Years are level 2, release dates level 3, and the New Features / Improvements
# / Bug Fixes headings inside each release level 4. Cap the table of contents at
# level 3 so it nests release dates under their year without repeating the same
# three subheadings once per release.
tableOfContents:
  maxHeadingLevel: 3
# Nothing above the first release is worth linking to, so drop the "Overview"
# entry as well and leave the dates on their own.
tocOverview: false
---
...
### 2026-09-23 <Badge text="Latest" variant="success" />

</summary>

#### New Features

- added T3 Code remote server support, including one-link pairing and OpenCode and Codex providers
- added a Neurodesktop Kasm image
- Jupyter webapps launchers now link to the webapp catalog

#### Improvements

- faster container startup by deferring non-essential services
- smaller image size and image validation before each release
- hardened desktop credentials and restricted notebook sudo access
- updated coding agents and image dependencies
- CVMFS health checks now name every missing container
- `ml help` and `ml keyword` are documented in the terminal banner

#### Bug Fixes

- fixed the Slurm dashboard launcher and job-history username
- fixed T3 Code startup behind JupyterHub 6
- fixed Webapps catalog order and section icon
- restored the IHEP CVMFS replica
</details>
