...
[NeurodeskAppX](/getting-started/app/neurodeskappx/) provides a portable Neurodesktop experience for users without privileged system access (root/admin permissions). It is powered by [CrumbleCracker](https://github.com/tinyrange/crumblecracker), an independent Linux virtual machine runtime that runs in the application process without requiring Docker, Podman, or a separate daemon.

NeurodeskAppX provides:

- Installation and execution without admin/root access
- A standalone graphical Neurodesktop application
- User-space networking and virtual machine management
- Portable storage or installation within the user's profile
- Support for Apple Silicon macOS, x86-64 Windows, and x86-64 Linux
- [Experimental OpenGL acceleration on Apple Silicon](/getting-started/app/neurodeskappx/#gpu-acceleration) in supporting builds
