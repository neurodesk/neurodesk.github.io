...
More information can be found at [Neurodesk App](/getting-started/app/neurodeskapp/).
...
If enabling `--nv` produces errors like this:
...
This means that the glibc versions inside the container and outside the container are not compatible. Use a newer container with a compatible glibc. Disabling `--nv` can prevent OpenGL GUIs from working on an NVIDIA-driven display. See [OpenGL GUIs on NVIDIA displays](/getting-started/neurocontainers/singularity/#opengl-guis-on-nvidia-displays).
...
If you need NVIDIA GPU compute or an OpenGL GUI on an NVIDIA-driven display, add `--nv`:
