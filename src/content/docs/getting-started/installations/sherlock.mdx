...
```bash
curl -J -O https://raw.githubusercontent.com/neurodesk/neurodesktop/refs/heads/main/scripts/connectSherlock.sh
```

The script checks that URL each time it starts and restarts itself if a newer
valid copy is available. Copies downloaded before the self-updater was added
need this download command once to move onto the automatic update path.
...
```bash
wget https://github.com/mathworks/MATLAB-extension-for-vscode/releases/download/v1.3.13/language-matlab-1.3.8.vsix
```
...
# Keep new installation files private to the shared group.
umask 0007
shared_group=$(stat -c '%G' "$GROUP_HOME")
...
# Normalize the ownership and permissions of files generated by the build.
chgrp -R "$shared_group" .
chmod -R o-rwx,g+rwX .
find . -type d -exec chmod g+s {} +

# $GROUP_HOME uses NFSv4 ACLs. Give new files group read and write access.
find . -type d -exec \
	nfs4_setfacl -a 'A:f:GROUP@:rwatTnNcy' {} +

# Give new directories group read, write, and traverse access. Inherit both rules.
find . -type d -exec \
	nfs4_setfacl -a 'A:d:GROUP@:rwaDxtTnNcy' {} +

# Confirm that the repository root contains both inherited ACL entries.
nfs4_getfacl .
...
Submit the current stable Neurodesktop version:

<Code code={neurodesktopUpdateCommand} lang="bash" mark={jupyterNeurodeskVersion} />
