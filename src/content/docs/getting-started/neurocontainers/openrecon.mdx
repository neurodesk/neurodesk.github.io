...
This checkout is for local package builds, which do not require a fork or a push to OpenRecon. To contribute changes directly to OpenRecon, follow [Fork OpenRecon for packaging changes](#fork-openrecon-for-packaging-changes) before cloning.
...
If you already cloned OpenRecon in section 4, reuse that checkout and activate its `.venv` environment. Local package builds do not require a fork or a push. To contribute changes directly to OpenRecon, follow [Fork OpenRecon for packaging changes](#fork-openrecon-for-packaging-changes).

Otherwise, clone the packaging repository next to your Neurocontainers checkout:
...
#### Configure a protocol to use the FIRE container

After installing the package, connect its ICE program to the protocol. The screenshots below show the setup for an image-to-image FIRE package.

1. In the protocol editor, find **Generic Views** under **Default AddIns** and add it to the protocol you want to use with FIRE.

   ![Protocol editor with Generic Views selected in the Default AddIns list](/docs/neurocontainers/fire/FIRE_0.JPG)

2. Check that **Generic Views** appears on the protocol entry.

   ![Protocol entry with the Generic Views add-in attached](/docs/neurocontainers/fire/FIRE_1.JPG)

3. Open the protocol's properties, select **Preview**, then click **AddIn Configuration...**.

   ![Protocol properties showing Preview and the AddIn Configuration button](/docs/neurocontainers/fire/FIRE_2.JPG)

4. In the **myExam Add-In Configurator**, select **Queue**. Under **ICE**, select **Use Customer Path**. In the example, this points to `C:\ProgramData\Siemens\Numaris\MriCustomer\Ice`.
5. Set **ICE program name** to the program supplied by your installed FIRE package. The screenshot shows an `openreconi2iexample` program with a name beginning `wip_070_fire_IceFireImageAddin_`. Choose the entry for your own package.

   ![Queue settings with Use Customer Path selected and the FIRE ICE program chosen](/docs/neurocontainers/fire/FIRE_3.JPG)

6. Close the configurator and protocol properties, then save the protocol. Test it with the installed FIRE package and check that it returns the expected image series.
...
### Fork OpenRecon for packaging changes

If you need to submit changes directly to OpenRecon, first fork [`neurodesk/openrecon`](https://github.com/neurodesk/openrecon/fork) into your GitHub account. Forking Neurocontainers does not create an OpenRecon fork. Replace `YOUR_GITHUB_USERNAME` below with your GitHub username.

For a new checkout, clone your fork from the directory containing your Neurocontainers checkout:

```bash
git clone https://github.com/YOUR_GITHUB_USERNAME/openrecon.git
cd openrecon
git remote add upstream https://github.com/neurodesk/openrecon.git
git switch -c myrecon-packaging
```

If you already cloned `neurodesk/openrecon`, keep your existing checkout and branch. After creating your fork on GitHub, run these commands inside that checkout:

```bash
git remote set-url origin https://github.com/YOUR_GITHUB_USERNAME/openrecon.git
git remote -v
```

The `origin` fetch and push URLs must point to your fork. Changing the remote preserves your local files and commits.

Review and commit the intended source changes, then push your current branch and configure tracking:

```bash
git push --set-upstream origin HEAD
```

Open a pull request from that branch in your fork to `neurodesk/openrecon:main`. Keep generated packages and private test data out of the commit.

## 10. Troubleshooting and scanner notes

### Git push fails in the OpenRecon checkout

`The current branch ... has no upstream branch` means Git has no tracking branch configured. `git push --set-upstream origin HEAD` configures tracking when the push succeeds.

If the push instead reports `Permission to neurodesk/openrecon.git denied` with HTTP 403, `origin` points to the Neurodesk repository and your account lacks write access. Follow [Fork OpenRecon for packaging changes](#fork-openrecon-for-packaging-changes) to create a fork and update the existing checkout's remote, then retry the push. Setting a tracking branch does not grant repository access.

For a normal recipe contribution, return to your Neurocontainers checkout and follow section 9. OpenRecon package builds alone do not require a push.

### Package behaviour and metadata

#### Available colour maps for LUT settings

The colour maps available for LUT settings are stored on the scanner host in:

```text
C:\Program Files\Siemens\Numaris\MriProduct\config\mri\ColorLUT
```

Most systems include these palette files:
...
:::danger[Protocols are deleted on scanner restart]
If a protocol has OpenRecon selected in the **Inline** tab and the corresponding OpenRecon container has been deleted, the scanner deletes that protocol on restart.
:::
